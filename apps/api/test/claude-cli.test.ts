...
import { PLUGIN_DIR, skillBody, skillPresent, SKILL_PATH } from "../src/jobs/claude-cli.js";
...
describe("the skill ships with the project", () => {
  it("is present and readable from the api package, not from ~/.claude", () => {
    ...
    expect(SKILL_PATH).toContain(join("apps", "api", "claude-plugin"));
    expect(SKILL_PATH.includes(`${sep}dist${sep}`)).toBe(false); // tsc does not copy .md — it must live outside dist
  });
